...
import type { AIResponse } from '../services/aiClient.js'
...
export const buildAnswer = (aiResponse: AIResponse | Error | undefined): string => {
  if (aiResponse instanceof Error) {
    return `${ERROR_ANSWER}\n\nError: ${aiResponse.message}`
  }
  if ((aiResponse == null) || !aiResponse.content) {
    return `${ERROR_ANSWER}\n\n${DISCLAIMER}`
  }
